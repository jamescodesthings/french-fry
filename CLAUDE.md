...
`french-fry-the-agent` is a static HTML/CSS/JS microsite served by a minimal Node.js/Express static file server. The server has no API routes or templating — it only serves files from `public/`. Docker Compose handles containerisation; Make wraps all common operations.
...
- All static assets must live under `public/`. The server will not serve files from outside this directory.
- `server.js` must stay thin — no business logic, no routes, no templates.
- CSS goes in `public/css/`, JS in `public/js/`. Do not put scripts or styles inline in HTML.
- The port is always `3000`. Change it only via the `PORT` env var — never hardcode an alternate.

## Adding pages

Drop a new `.html` file in `public/` and link to it from `index.html`. No server changes needed.
